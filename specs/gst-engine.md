...
**GE-18 — OTP auth has a session lifetime and this shapes the workflow.** A
CA cannot batch-file 30 clients without 30 OTPs, each needing the *client's*
registered mobile. Practical consequences: the console must support a
session-per-client model, show which sessions are live, and let the CA queue
work to execute while a session is valid. **Get the exact session duration and
whether it is re-usable across calls — it determines whether bulk filing is
feasible at all.** Open question §14.2.
...
| T-22 | 30 clients due for GSTR-1 | Bulk file | Per-client OTP sessions handled; 30 audit rows, shared batch id |
...
**15.2 OTP session mechanics.** The critical operational question (GE-18). How
long does a taxpayer session last, is it reusable across calls, and can a CA
realistically file for 30 clients in a sitting? **This determines whether bulk
filing is a feature or a fantasy.** Worth testing directly against Sandbox.
